...
/* 데이터 접근 계층 — 동적(로컬 서버) / 정적(GitHub Pages) 모두 지원.
   정적 모드: index.html에서 window.__STATIC__=true 설정 + ./api/*.json 번들 사용. */
const STATIC = typeof window !== "undefined" && window.__STATIC__;
const _bundle = {};
const bundle = (name) =>
  (_bundle[name] = _bundle[name] || fetch(`./api/${name}.json`).then((r) => r.json()));

const getCountries = () => (STATIC ? bundle("countries") : api("/api/countries"));
const getFoods = async (cid) =>
  STATIC ? (await bundle("foods"))[cid] || [] : api("/api/foods?country=" + encodeURIComponent(cid));
const getRecommend = async (id) =>
  STATIC ? (await bundle("reco"))[id] : api("/api/recommend?id=" + id);
const getPlaces = async (food) =>
  STATIC
    ? (await bundle("places"))[food] || { restaurants: [], specialties: [],
        links: { naver: "https://map.naver.com/p/search/" + encodeURIComponent(food + " 맛집"),
                 kakao: "https://map.kakao.com/?q=" + encodeURIComponent(food + " 맛집") } }
    : api("/api/places?food=" + encodeURIComponent(food));
...
async function loadFoods(country) {
  ...
  goStep(2);
  const foods = await getFoods(country.id);
